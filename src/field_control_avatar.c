...
void NextCostume()
{
	...
	if(costume < NUM_COSTUMES)
		...
		VarSet(VAR_COSTUME_NUMBER, EMERALD_COSTUME);
}

void SetCostume(u8 costume)
{
	if(costume < NUM_COSTUMES)
		VarSet(VAR_COSTUME_NUMBER, costume);
	else
...
int ProcessPlayerFieldInput(struct FieldInput *input)
{
    ...
    if (input->pressedStartButton)
    {
        ...
        {
			if(gSaveBlock1Ptr->location.mapGroup  != MAP_GROUP(INSIDE_OF_TRUCK)
				&& !gPlayerAvatar.preventStep){
...
static const u8 *GetInteractedObjectEventScript(struct MapPosition *position, u8 metatileBehavior, u8 direction)
{
	...
	else if (objectEventId == POF_GetFollowerObjectId()) //tx_pokemon_follower
        script = POF_GetFollowerScriptPointer();
    else
...
static bool8 TryStartStepCountScript(u16 metatileBehavior)
{
    ...
    if (!(gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_FORCED_MOVE) && !MetatileBehavior_IsForcedMovementTile(metatileBehavior))
    {
        ...
        {
            //RustboroCity_Gym_EventScript_GymGuide_New
            ScriptContext1_SetupScript(EventScript_Gym_Attendant_Call);
